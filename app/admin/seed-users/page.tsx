    ...
    <div style={{ padding: "20px", maxWidth: "900px", margin: "0 auto" }}>
      ...
      >
        ...
        <small style={{ color: "#666" }}>
          ℹ️ هذا المفتاح مخزن في ADMIN_SEED_KEY بـ .env.local
        </small>
      </div>
      ...
      >
        {DEFAULT_USERS.map((user, idx) => (
          ...
          >
            <h3 style={{ margin: "0 0 10px 0", fontSize: "16px" }}>{user.full_name}</h3>
            <div style={{ fontSize: "12px", color: "#666", marginBottom: "10px" }}>
              <p style={{ margin: "5px 0" }}>📧 {user.email}</p>
              <p style={{ margin: "5px 0" }}>📱 {user.phone}</p>
              <p style={{ margin: "5px 0" }}>🔑 {user.password}</p>
              <p style={{ margin: "5px 0" }}>
                👤 {user.user_type === "provider" ? "🏥 مزود" : "👨‍🔬 مريض"}
              </p>
            </div>
            <button
              onClick={() => seedSingleUser(user)}
              disabled={loading || !adminKey}
              style={{
                width: "100%",
                padding: "10px",
      ...
      <div style={{ marginTop: "40px", padding: "20px", backgroundColor: "#e3f2fd", borderRadius: "8px" }}>
        <h3>📖 الخطوات:</h3>
        <ol style={{ direction: "rtl" }}>
          <li>ابحث عن ADMIN_SEED_KEY في ملف .env.local</li>
          <li>أدخل المفتاح في الحقل أعلاه</li>
          <li>اضغط "إضافة جميع المستخدمين" أو اختر مستخدماً واحداً</li>
          <li>انتقل إلى صفحة تسجيل الدخول وادخل الببانات</li>
        </ol>
      </div>
    </div>
  );
